...
# 单细胞数据分析流程 - 第五步（CD45+ 免疫细胞类型注释）
#
# 本脚本的功能：
#   1) 读取第四步整合聚类后的对象（04_CD45_integrated.rds）
#   2) 用 AddModuleScore 对每个 cluster 做免疫 marker 打分（T/CD8T/CD4T/NK/B/
#      浆细胞/单核/巨噬/DC/肥大/中性粒 等常见免疫细胞亚群 marker）
#   3) 可视化：关键 marker 的 UMAP 表达图 + cluster × 细胞类型打分热图，
#      帮助确认每个 cluster 的细胞身份（"先看结果再定注释"）
#   4) 注释流程（手动注释，参考 Lesson 2~6 教程与 细胞注释.md）：
#       - 首次运行：根据打分自动生成"注释建议表" celltype_map_template.csv
#         （cluster | top_hit 建议 | celltype 待填），你根据打分热图/marker UMAP
#         确认后编辑该表，保存为 celltype_map.csv
#       - 再次运行：脚本自动读取 celltype_map.csv 并应用注释到每个 cluster
#   5) 输出：注释后的 Seurat 对象（05_annotated.rds）、打分矩阵 CSV、
#      细胞类型组成统计、以及注释 UMAP / 每细胞类型 marker 点图
#
# ---------------------------------------------------------------------
# 本版主要改动（相对旧版）：
#   * 以 SeuratExtend（huayc09/SeuratExtend）作为【主要绘图方式】：
#       - DimPlot2       替代 Seurat::DimPlot    （增强降维图）
#       - FeaturePlot3.grid 替代 逐基因 FeaturePlot + wrap_plots（多基因 UMAP 表达网格）
#       - DotPlot2       替代 Seurat::DotPlot    （增强点图，支持分组 features）
#       - SeuratExtend::Heatmap 替代 pheatmap    （打分热图；pheatmap 作兜底）
#       - ClusterDistrBar 替代 ggplot 柱状图     （各组细胞类型组成；ggplot 作兜底）
#   * 所有可调参数集中到 ## 0. 配置，并对每个默认值注释"为什么这样设"
#   * 全脚本逐行中文注释
#   * 细胞注释信息（CELLTYPE_MARKERS）按 细胞注释.md + Lesson 5/6 整理
#   * 每细胞类型 marker 点图：先放注释文件分子（≤5），再放该类型 top5 差异基因，
#     合计约 10 个/类型（见 7d 节）
#   * 代码重新排序：每一节的"分析"与"对应作图"放在同一节内
#   * 【重构】把原版超长的 if/tryCatch 块（最长 100+ 行）拆分为 1b 节的独立小函数
#     （plot_score_heatmap / plot_marker_umap / apply_celltype_map / write_map_template /
#      plot_annotation_umap / write_composition_stats / plot_composition /
#      plot_celltype_dotplot / plot_firstrun_dotplot）：函数名即任务名，改哪里找哪个函数；
#     行为与输出图完全不变（本次仅重构，不改图）
#
# 用法：
...
#   06_marker_dotplot.pdf            - 每细胞类型核心 marker 表达点图（应用注释后生成）
#   02_celltype_summary.csv / 03_celltype_by_group.csv - 组成统计表（应用注释后生成）
...
# 原因：手动注释需人机协同，模板/正式表用同一文件名，逻辑清晰。
...
# 用哪一列作为聚类分组（04 步的主 cluster 列，默认 0.2 分辨率）。
...
if (file.exists(MANUAL_MARKERS_FILE)) {
  ...
  cat("警告: 未找到 manual_markers.R，手动点图将使用 CELLTTYPE_MARKERS 兜底。\n")
  MANUAL_MARKERS <- list()
}
