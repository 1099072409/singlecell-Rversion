...
#      （用于先看图选 res）。如需在停止点补充 Harmony 去批次前后对比图，
#      把 INCLUDE_HARMONY_BEFORE_AFTER 改为 TRUE 即可。
#   多分辨率 UMAP 画图代码遵循 Lesson-2-Basic-Single.md：
#     DimPlot(label = TRUE, group.by = "cluster")
#
# 功能：
#   1) 读取第三步提取的 CD45+ 细胞对象（03_CD45_positive.rds）
#   2) 按指定条件剔除样本（EXCLUDE_SAMPLES，防御性步骤；已剔除则确认跳过）
#   3) 标准化：NormalizeData（LogNormalize）+ FindVariableFeatures（vst，3000 高变基因）
#      + ScaleData（可选回归线粒体比例，对齐 yijian.R）
#   4) 去批次：RunPCA 后 RunHarmony（以 sample 为批次变量），消除样本间技术差异
#   4b) PC 选择诊断：ElbowPlot + DimHeatmap（看每个 PC 的 top 基因），然后
#       根据累积方差/标准差自动确定 DIMS（也支持手动覆盖）
#   5) 降维可视化：RunUMAP（基于 harmony）、RunTSNE（可选）
#   6) 聚类：FindNeighbors + FindClusters，支持多分辨率，各分辨率结果保存在
#      meta.data 的 cluster_0.1 ~ cluster_1.0 列中，并指定默认分辨率（0.4）作为主 cluster 列
#   7) 输出：整合聚类后的 Seurat 对象（04_CD45_integrated.rds）、聚类统计表
#      （01_clustering_summary.csv / 02_cluster_by_sample.csv / 03_resolution_summary.csv）、
#      以及 PCA 诊断图（Elbow / DimHeatmap）、多分辨率 UMAP 拼图
#
# 用法：
#   在 RStudio 中打开本文件并点击 Source，或在命令行执行：
#     "C:/Program Files/R/R-4.4.3/bin/Rscript.exe" 04_integration_clustering_exclude.R
...
UMAP_NEIGHBORS  <- 110      # n.neighbors：局部邻域大小（越小越强调局部结构）
UMAP_MIN_DIST   <- 1.1     # min.dist：嵌入点间最小距离（越大越分散；0.1/0.3/0.5 可先试图再选）
...
PLOT_HARMONY_CONVERGENCE <- FALSE # 是否绘制 Harmony 收敛诊断图（图 10_Harmony_convergence.pdf）。
                           #   exclude 版默认 FALSE：本次只跑到多分辨率 UMAP 图，收敛图不生成；
                           #   同时避免 RunHarmony 以列表形式返回（省内存、更稳）。
                           #   完整流程需要时改回 TRUE（注意 harmony 1.2.4 下可能绘制失败，脚本会自动跳过）。
...
DEFAULT_RESOLUTION <- 0.4             # 默认分辨率：其聚类结果写入主 cluster 列（可在看完
                                      #   02_UMAP_multi_resolution.pdf 后调整再重跑）

# ---- 其他 ----
RUN_TSNE <- FALSE      # 是否运行 tSNE 降维。exclude 版默认 FALSE：本次只跑到多分辨率 UMAP，
                       #   tSNE 与其图（08_TSNE_by_cluster.pdf）均不需要，关闭可省 5~15 分钟与内存；
                       #   需要完整结果时改回 TRUE。
SEED     <- 123        # 随机种子（保证 UMAP/聚类结果可复现）

# ---- 停止点开关（exclude 版专项）----
STOP_AFTER_MULTI_RES_UMAP <- FALSE # FALSE：跳过停止点，执行完整 9.2~9.8 可视化并保存整合对象(rds)
                                   #   保存主 cluster 列 + 聚类统计表 + 整合对象(rds) 即停止，
                                   #   不生成 9.2~9.7 默认分辨率图（用于先看图选 res）；FALSE 则完整跑完。
INCLUDE_HARMONY_BEFORE_AFTER <- FALSE # 停止点是否额外绘制 Harmony 去批次前后对比图
                                      # （09_Harmony_before_after_UMAP.pdf）。
                                      # 默认 FALSE：本次只执行至 02_UMAP_multi_resolution.pdf；
                                      # 需要该诊断图时改为 TRUE（会多跑一次 PCA-UMAP，约数分钟）。
...
if (RUN_HARMONY) {                                 # 开启去批次时
  ...
  harmony_args <- list(                            # 用列表统一管理参数（便于条件追加）
    ...
    theta = 4,           # 默认 2，增加到 3-5
    max.iter.harmony = 40,# 增加迭代次数
...
# 画图代码遵循 Lesson-2-Basic-Single.md 的规范：
#   DimPlot(pbmc, label = TRUE, group.by = "cluster")
# 逐分辨率把 cluster_0.1 ~ cluster_1.0 当作 "cluster" 列绘制 UMAP（label 标注簇号），
# 再用 patchwork 拼图直观对比不同分辨率下的簇划分，辅助选择合适的分辨率。
if (length(RESOLUTIONS) > 1) {                     # 多于 1 个分辨率时
  ...
  plist <- lapply(RESOLUTIONS, function(r) {       # 逐分辨率生成 UMAP 图
    DimPlot(obj, label = TRUE,                     # Lesson-2 画法：label 标注簇号
...
# ==== 9.1b 停止点：执行至多分辨率 UMAP（exclude 版专项） ====
# 按配置 STOP_AFTER_MULTI_RES_UMAP=TRUE：生成 02_UMAP_multi_resolution.pdf 后，
# 立即设置默认分辨率主 cluster 列、写聚类统计表、保存整合对象（rds），然后停止，
# 不生成 9.2~9.7 基于默认分辨率的图（by sample/group/cluster/split/柱状图/tSNE）。
# 是否在停止点补充 Harmony 去批次前后对比图（09_Harmony_before_after_UMAP.pdf）
# 由 INCLUDE_HARMONY_BEFORE_AFTER 控制（默认 FALSE，本次只执行至 02_UMAP）。
if (STOP_AFTER_MULTI_RES_UMAP) {
  cat("\n==== 已生成 02_UMAP_multi_resolution.pdf，按配置保存结果并停止 ====\n")
  # Harmony 去批次前后对比图（09_Harmony_before_after_UMAP.pdf，核心诊断图，可选）：
  # 左 = 未整合（基于 PCA 的 UMAP），右 = 整合后（基于 Harmony 的 UMAP），均按样本着色。
  # 判读标准：左图样本各自成团（批次效应明显）、右图样本混合均匀即去批次有效。
  if (INCLUDE_HARMONY_BEFORE_AFTER && RUN_HARMONY && "harmony" %in% names(obj@reductions)) { # 按配置开启时
  ...
  # 指定默认分辨率作为主 cluster 列（与完整版第 8 节逻辑一致），并设为当前 idents
  ...
  cat(sprintf("默认分辨率（res=%s）聚类结果：\n", DEFAULT_RESOLUTION)) # 提示
  print(table(obj$cluster))                           # 各 cluster 细胞数
  # 聚类统计表（与完整版第 8 节一致）：
  #   8.1 主 cluster 汇总：cluster / 细胞数 / 占比
  ...
  cat("  已保存: 01_clustering_summary.csv / 02_cluster_by_sample.csv / 03_resolution_summary.csv\n")
  # 保存整合对象：含全部 cluster_0.1~1.0 列 + 主 cluster 列 + pca/harmony/umap 降维
  saveRDS(obj, file.path(OUT_DIR, "04_CD45_integrated.rds"))
  cat(sprintf("  已保存: %s\n", file.path(OUT_DIR, "04_CD45_integrated.rds")))
  cat(sprintf("\n【本次执行至多分辨率 UMAP%s】\n",
              if (INCLUDE_HARMONY_BEFORE_AFTER) " + Harmony 前后对比图" else ""))
  cat("  已生成: 01_PCA_elbow.pdf / 01_PCA_dimheatmap.pdf / 02_UMAP_multi_resolution.pdf\n")
  if (INCLUDE_HARMONY_BEFORE_AFTER) cat("          09_Harmony_before_after_UMAP.pdf\n")
  cat("  已保存: 01_clustering_summary.csv / 02_cluster_by_sample.csv / 03_resolution_summary.csv\n")
  cat("          04_CD45_integrated.rds（整合聚类对象，供 05 细胞注释直接使用）\n")
  cat("  其余默认分辨率图（by sample/group/cluster/split/柱状图/tSNE/收敛图等）未生成。\n")
  cat("  如需完整结果：把 CONFIG 中 STOP_AFTER_MULTI_RES_UMAP 改为 FALSE（并按需将\n")
  cat("  RUN_TSNE 改回 TRUE、INCLUDE_HARMONY_BEFORE_AFTER 改回 TRUE）后重跑本脚本即可。\n")
  quit(save = "no", status = 0)                      # 干净退出，不再执行后续代码
}
...
save_pdf(p_split, "06_UMAP_by_sample_split.pdf", 18, 12) # 保存（大画布容纳 18 面板）
