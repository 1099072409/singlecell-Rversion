...
#   与 04_integration_clustering.R 逻辑、参数、输出完全一致，仅三点差异：
#   a) 输入改为同目录下的 exclude/03_CD45_positive.rds（已剔除 5 个样本）
#   b) 输出写到脚本自身所在目录（exclude 文件夹）
#   c) 默认 STOP_AFTER_MULTI_RES_UMAP=TRUE：跑到多分辨率 UMAP 图
#      （02_UMAP_multi_resolution.pdf）并补充 Harmony 去批次前后对比图
#      （09_Harmony_before_after_UMAP.pdf）后保存 rds + 统计表即停止，
#      不生成后续默认分辨率图（用于先看图选 res）
...
# ---- 停止点开关（exclude 版专项）----
STOP_AFTER_MULTI_RES_UMAP <- TRUE # TRUE：跑到 02_UMAP_multi_resolution.pdf 并补充
                                  # 09_Harmony_before_after_UMAP.pdf 后，保存 rds + 统计表即停止，
...
# ==== 9.1b 停止点：跑到多分辨率 UMAP + Harmony 前后对比图（exclude 版专项） ====
# 按配置 STOP_AFTER_MULTI_RES_UMAP=TRUE：生成 02_UMAP_multi_resolution.pdf 与
# 09_Harmony_before_after_UMAP.pdf（用户指定需要 Harmony 的图）后，
...
if (STOP_AFTER_MULTI_RES_UMAP) {
  cat("\n==== 已生成 02_UMAP_multi_resolution.pdf，按配置补充 Harmony 图后停止 ====\n")
  # Harmony 去批次前后对比图（09_Harmony_before_after_UMAP.pdf，核心诊断图）：
